# Spec: vault-onboarding

## Purpose

The `vault-onboarding` capability manages the first-run experience for users who have not yet configured a vault. It is responsible for detecting whether a vault is configured on startup, presenting an onboarding screen with a directory picker when no vault is found, invoking vault initialisation after a directory is selected, and transitioning to the main window on success.
